...
!    -------------- CALCULATE & PRINT CHARGE-DISTANCE PROFILES -----------------
   ...
   370 continue
       ...
       ! initial guess of concentrations in layer
       do j = 1, nspec
       
         zconc(i,j)   = gamma(j) *  bulkc(j)                   ! mol dm**-3
         conc(j) = zconc(i,j)
       ...
       do j = 1, nspec
         ...
         if (convergence(j).eqv..false.) goto 370

       end do

       do j = 1, nspec

         zconc(i,j)   = conc(j)* P ** zchge(j)
...
!    ***************************************************************************
     ...
     SUBROUTINE temp_activity
     USE header_file, only: gamma_old,gamma,double,zchge,pitzer_b0, &
    &  pitzer_b1,pitzer_C,pitzer_phi,pitzer_lamda,pitzer_psi,nspec,P,conc,&
    &  convergence
!    ***************************************************************************
     implicit none
     integer :: i,j,k
     real(kind=double) :: alpha, alpha1, gam_sum, zchge2, z_val, dh_lim
     real(kind=double) :: pitzer_ba, pitzer_bb, gfunc, ion_strength
     real(kind=double) :: loggam
     ...
     ion_strength = 0.0d0
     do i = 1, nspec
       ion_strength =  ion_strength + 0.5 * conc(i) * zchge(i)**2 * P ** zchge(i)
       z_val = z_val + conc(i) * zchge(i) * P ** zchge(i)
     end do
     ...
     do i = 1, nspec
      ...
      &           + 1.667*dlog(1+1.2*sqrt(ion_strength)))
       ...
       if (zchge(i).gt.0.0d0) then
         ...
         do j = 1, nspec
           ...
           if (zchge(j).lt.0.0d0) then
             ...
             gam_sum = gam_sum + conc(j) * (2.0d0 * pitzer_ba + z_val * pitzer_c(i,j))
             ! inner cation loop
             do k = 1, nspec
               ...
               if (zchge(k).gt.0.0d0) then
                 ...
                 gam_sum = gam_sum + abs(zchge(i)) * conc(j) * conc(k) * pitzer_c(j,k)
                 gam_sum = gam_sum + zchge2 * conc(j) * conc(k) * &
               &                (pitzer_bb + z_val * pitzer_c(j,k) / 2.0)
                 gam_sum = gam_sum + conc(k) * conc(j) * pitzer_psi(i,j,k)
               ! inner anion loop
               else if (zchge(k).lt.0.0d0) then
                   gam_sum = gam_sum + zchge2 * conc(j) * conc(k) * pitzer_phi(j,k)
                   gam_sum = gam_sum + conc(j) * conc(k) * pitzer_psi(i,j,k)
               end if
           ...
           ! cationic species
             ...
             gam_sum = gam_sum + 2.0d0 * conc(j) * pitzer_phi(i,j)
             ! inner cation loop
             do k = 1, nspec
               if (zchge(k).gt.0.0d0) then
                 gam_sum = gam_sum + zchge2 * conc(j) * conc(k) * pitzer_phi(j,k)
           ...
           ! neutral species
             ...
             gam_sum = gam_sum + 2.0d0 * conc(j) * pitzer_lamda(i,j)
           end if
       ...
       ! computing the activity coefficient of a neutral species
         ...
         do j = 1, nspec
           gam_sum = gam_sum * conc(j) * pitzer_lamda(i,j)
         end do
